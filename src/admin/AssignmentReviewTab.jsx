...
export default function AssignmentReviewTab({ assignments, setAssignments, setToast }) {
  ...
  const ALL_COURSES = useMemo(() => [...new Set(assignments.map(a => a.course))], [assignments]);
  ...
  const handleApprove = id => {
    ...
    setToast({ msg: "Assignment approved successfully.", type: "success" });
  };
  ...
  const handleNotify = id => {
    ...
    setToast({ msg: "Teacher notified via email and in-app message.", type: "success" });
  };
        ...
        <div style={{ background: "white", borderRadius: 20, padding: 24, border: "1px solid #f1f5f9", boxShadow: "0 4px 20px rgba(0,0,0,0.06)", marginBottom: 18 }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: 16, marginBottom: 16, paddingBottom: 16, borderBottom: "1px solid #f3f4f6" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                <span style={{ fontSize: 12, color: "#6b7280" }}>Teacher: {a.teacher}</span>
                <span style={{ fontSize: 12, color: "#6b7280" }}>Course: {a.course}</span>
                <span style={{ fontSize: 12, color: "#6b7280" }}>Batch: {a.batch}</span>
                <span style={{ fontSize: 12, color: "#6b7280" }}>Submitted: {a.submitted}</span>
              </div>
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          {a.status === "approved" && (
            <div style={{ marginTop: 14, padding: "10px 14px", background: "#d1fae5", borderRadius: 10, fontSize: 12, color: "#065f46", border: "1px solid #86efac" }}>
              Assignment approved and marks finalized.
            </div>
          )}
        </div>
            ...
            <div style={{ background: "white", borderRadius: 16, padding: 20, border: "1px solid #f1f5f9", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
              ...
              {aiLoading && (
                <div style={{ marginTop: 10, padding: 14, background: "#f5f3ff", borderRadius: 10, border: "1px solid #c4b5fd", fontSize: 12, color: "#7c3aed" }}>
                  <div style={{ fontWeight: 700, marginBottom: 4 }}>AI is analyzing rubric scores...</div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 12, color: "#6b7280", marginTop: 3, display: "flex", gap: 12, flexWrap: "wrap" }}>
                    ...
                    <span>Course: {a.course}</span>
                    <span>Batch: {a.batch}</span>
                    <span>Reviewer: {a.trainer || "Unassigned"}</span>
                ...
                <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                  ...
                  {!a.notified && (a.status === "approved" || a.status === "revision") && (
                    <button onClick={() => handleNotify(a.id)} style={{ ...AR_BTN_GHOST, color: "#8b5cf6", borderColor: "#c4b5fd" }}>Notify</button>
                  )}
                </div>
              </div>
            </div>
