...
export default function GeotagAttendance({ user }) {
  ...
  const buildAttendanceMapFromLogs = (logs = []) => {
    return logs.reduce((map, log) => {
      if (!map[log.dateKey]) map[log.dateKey] = {};
      if (log.actionType === "checkin") {
        map[log.dateKey].checkedIn = true;
        map[log.dateKey].checkInTime = log.checkInTime;
      }
      if (log.actionType === "checkout") {
        map[log.dateKey].checkedOut = true;
        map[log.dateKey].checkOutTime = log.checkOutTime;
      }
      return map;
    }, {});
  };

  const toHistoryEntry = (log) => ({
    id: log._id,
    type: log.actionType,
    date: log.dateLabel,
    time: log.actionType === "checkin" ? log.checkInTime : log.checkOutTime,
    coords: log.coords,
    status: log.status,
    snapshot: log.snapshot,
    distanceOffset: log.distanceOffset,
    dateKey: log.dateKey
  });
  ...
  useEffect(() => {
    ...
    const loadLogs = async () => {
      try {
        const response = await fetch(`${API_BASE}/geotag-attendance/${teacherId}`);
        if (!response.ok) throw new Error("Failed to load geotag attendance records");
        const logs = await response.json();
        if (!active) return;
        const normalizedLogs = logs.map(toHistoryEntry);
        setHistoryLogs(normalizedLogs);
        setAttendanceMap(buildAttendanceMapFromLogs(logs));
      } catch (error) {
        console.error("Failed to load geotag attendance:", error);
      }
    };
  ...
  const saveGeotagEvent = async (type, lat, lng, snapshot, timeStr, dateStr, coordStr, dist) => {
    if (!teacherId) throw new Error("Teacher session missing. Please log in again.");

    const endpoint = type === "checkin" ? "checkin" : "checkout";
    const response = await fetch(`${API_BASE}/geotag-attendance/${teacherId}/${endpoint}`, {
      method: "POST",
    ...
    if (!response.ok) {
      ...
      throw new Error(data.error || "Failed to store geotag attendance");
    }
  ...
  const processAttendance = (type, lat, lng) => {
    setTimeout(() => {
      ...
      saveGeotagEvent(type, lat, lng, snapshot, timeStr, dateStr, coordStr, dist)
        .then(savedLog => {
          const entry = toHistoryEntry(savedLog);
          setAttendanceMap(prev => {
            const next = { ...prev };
            if (!next[todayKey]) next[todayKey] = {};
            if (type === "checkin") {
              next[todayKey].checkedIn = true;
              next[todayKey].checkInTime = timeStr;
            } else {
              next[todayKey].checkedOut = true;
              next[todayKey].checkOutTime = timeStr;
            }
            return next;
          });
          setHistoryLogs(prev => [entry, ...prev.filter(log => !(log.dateKey === todayKey && log.type === type))]);
          setStatusReport({
            success: true,
            type,
        ...
        .catch(error => {
          setErrorAlert(error.message || "Failed to store attendance.");
        })
        .finally(() => {
          setLoading(false);
          setActionType(null);
                ...
                <button
                  onClick={() => {
                    if (window.confirm("Clear all session logs?")) {
                      fetch(`${API_BASE}/geotag-attendance/${teacherId}/all`, { method: "DELETE" })
                        .then(() => {
                          setHistoryLogs([]);
                          setAttendanceMap({});
                        })
                        .catch(() => setErrorAlert("Failed to clear geotag logs."));
                    }
                  }}
