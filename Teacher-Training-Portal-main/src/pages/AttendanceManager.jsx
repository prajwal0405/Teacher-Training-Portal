...
import { SectionCard, S, Badge, Modal } from "../components/Shared";
...
export default function AttendanceManager({ user }) {
  ...
  const loadHistory = async () => {
    if (!teacherId) return;
    try {
      const response = await fetch(`${API_BASE}/daily-attendance/${teacherId}?subject=${encodeURIComponent(subject)}`);
      if (response.ok) {
        const sheets = await response.json();
        setHistorySheets(sheets);
      }
    } catch (err) {
      console.error("Failed to load historical sheets:", err);
    }
  };
    ...
    const loadSheet = async () => {
      try {
        const response = await fetch(
          `${API_BASE}/daily-attendance/${teacherId}/sheet?subject=${encodeURIComponent(subject)}&date=${encodeURIComponent(selectedDate)}&teacherEmail=${encodeURIComponent(teacherEmail)}`
        );

        if (response.status === 404) {
          if (!isMounted) return;
          try {
            const studentResponse = await fetch(`${API_BASE}/students/${teacherId}`);
            if (studentResponse.ok) {
              const studentList = await studentResponse.json();
              const roster = studentList.map((st, index) => ({
                rollNo: index + 1,
                name: st.name,
                status: "P"
            ...
            console.error("Failed to load students roster fallback:", studentErr);
          }
        ...
        if (!response.ok) {
          throw new Error("Failed to load attendance sheet");
        }

        const sheet = await response.json();
        if (!isMounted) return;

        const roster = [...(sheet.roster || [])].sort((a, b) => a.rollNo - b.rollNo);
        const init = {};
        roster.forEach(st => {
          init[st.rollNo] = st.status || "P";
        });

        setStudents(roster);
        setAttendanceDict(init);
        setIsSavedRecord(Boolean(sheet.isLocked));
      } catch (error) {
  ...
  }, [teacherId, teacherEmail, subject, selectedDate]);
  ...
  const buildRosterPayload = (roster = students, statuses = attendanceDict) =>
    roster
      .map(st => ({
        rollNo: st.rollNo,
        name: st.name,
        status: statuses[st.rollNo] || st.status || "P"
      }))
      .sort((a, b) => a.rollNo - b.rollNo);

  const saveSheetToServer = async ({ roster = students, statuses = attendanceDict, locked = false, notes = "" } = {}) => {
    if (!teacherId) throw new Error("Teacher session missing. Please log in again.");

    const response = await fetch(`${API_BASE}/daily-attendance/${teacherId}/sheet`, {
      method: "PUT",
      ...
      body: JSON.stringify({
        ...
        roster: buildRosterPayload(roster, statuses),
        isLocked: locked,
        notes
      })
    });
    ...
    if (!response.ok) {
      ...
      throw new Error(data.error || "Failed to save attendance sheet");
    }
  ...
  const doSendOtp = async (studentName, oldStatus, newStatus) => {
    ...
    otpRef.current = otp;
    console.log(`[Dev Mode] Verification OTP for ${studentName}: ${otp}`);
    setOtpStep("sending");
    try {
      ...
      setOtpError(`Failed to send OTP via EmailJS. (Demo Bypass OTP: ${otp})`);
      setOtpExpiry(Date.now() + OTP_EXPIRY_MS);
      setOtpStep("input");
      setTimeout(() => otpInputRefs.current[0]?.focus(), 100);
      startCooldown();
  ...
  const handleStatusToggle = (rollNo, currentStatus) => {
    ...
    if (isSavedRecord) {
      ...
      const nextAttendance = { ...attendanceDict, [rollNo]: nextStatus };
      setAttendanceDict(nextAttendance);
      saveSheetToServer({ roster: students, statuses: nextAttendance, locked: false }).catch(error => {
        console.error("Failed to save draft attendance:", error);
        triggerToast(error.message || "Failed to save draft attendance.", true);
      });
    }
  };
  ...
  const handleVerifyOtp = () => {
    ...
    setTimeout(() => {
      if (entered === otpRef.current) {
        if (pendingChange) {
          const nextAttendance = { ...attendanceDict, [pendingChange.rollNo]: pendingChange.nextStatus };
          const nextRoster = students.map(st => (
            st.rollNo === pendingChange.rollNo ? { ...st, status: pendingChange.nextStatus } : st
          ));
          saveSheetToServer({ roster: nextRoster, statuses: nextAttendance, locked: true })
            .then(() => {
              setAttendanceDict(nextAttendance);
            ...
            .catch(error => {
              setOtpStep("input");
              setOtpError(error.message || "Failed to save attendance changes.");
            });
          return;
        }
        setShowOtpModal(false);
  ...
  const handleAddStudent = async (e) => {
    ...
    const sorted  = [...students, { name: nameTrimmed }].sort((a, b) => a.name.localeCompare(b.name));
    const list    = sorted.map((st, i) => ({ rollNo: i + 1, name: st.name }));
    const added = list.find(s => s.name === nameTrimmed);
    const nextAttendance = { ...attendanceDict };
    if (added) nextAttendance[added.rollNo] = "P";

    try {
      // Register student globally in database so they persist for all future dates
      ...
      if (!studentRes.ok) {
        throw new Error("Failed to register child in database");
      }

      await saveSheetToServer({ roster: list, statuses: nextAttendance, locked: false });
      setStudents(list);
      setAttendanceDict(nextAttendance);
      setNewStudentName("");
      setShowAddModal(false);
      loadHistory();
      triggerToast("Child registered globally and roll numbers re-sorted!");
    } catch (error) {
      triggerToast(error.message || "Failed to register child.", true);
    }
  };
  ...
  const handleSaveSheet = async () => {
    try {
      const savedSheet = await saveSheetToServer({ roster: students, statuses: attendanceDict, locked: true });
      setIsSavedRecord(true);
      if (savedSheet && savedSheet.roster) {
        const roster = [...savedSheet.roster].sort((a, b) => a.rollNo - b.rollNo);
        const init = {};
        roster.forEach(st => {
          init[st.rollNo] = st.status || "P";
        });
      ...
      triggerToast(error.message || "Failed to archive attendance sheet.", true);
    }
  };
  ...
  const handleClearSheetRecord = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE}/daily-attendance/${teacherId}/sheet`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ subject, sheetDate: selectedDate })
      });
      ...
      triggerToast(error.message || "Failed to delete archived record.", true);
    }
  };
  ...
  return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      ...
      <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
        ...
        {/* Main Content Area: Date Picker + Roster */}
                ...
                {isSavedRecord
                  ? (
                    <span
                      ...
                      title="Click to view history list"
                    >
                      📝 Reviewing Saved Sheet History 🔍
                    </span>
                  )
        ...
        {/* History Area: Search through past sheets */}
        <div style={{ flex: "0 0 320px", minWidth: 300 }}>
          <SectionCard title="🔍 Past Historical Sheets">
            <div style={{ marginBottom: 12 }}>
              <input
                type="text"
                placeholder="Search sheets by date..."
                value={historySearch}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, maxHeight: 420, overflowY: "auto", paddingRight: 4 }}>
              {historySheets.length === 0 ? (
                <p style={{ fontSize: 12, color: "#9ca3af", fontStyle: "italic", textAlign: "center", padding: "12px 0" }}>
                  No past sheets found.
                </p>
              ) : (
                historySheets
                  .filter(sheet => {
                    ...
                    return sheet.sheetDate.toLowerCase().includes(query) || (sheet.notes && sheet.notes.toLowerCase().includes(query));
                  })
                  .map(sheet => {
                    const presentCount = sheet.roster.filter(s => s.status === "P").length;
                    const total = sheet.roster.length;
                    const isSelected = sheet.sheetDate === selectedDate;
                    ...
                    return (
                      ...
                      >
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
                          <span style={{ fontSize: 13, fontWeight: 700, color: "#1c1917" }}>📅 {sheet.sheetDate}</span>
                          {sheet.isLocked ? (
                            <Badge children="Archived" color="#047857" bg="#d1fae5" />
                          ) : (
                            <Badge children="Draft" color="#b45309" bg="#fef3c7" />
                          )}
                        </div>
                        <div style={{ fontSize: 11, color: "#64748b", marginBottom: 8 }}>
                          Roster: {total} children ({presentCount} present)
                        </div>
                        <button
                          onClick={() => setSelectedDate(sheet.sheetDate)}
                          style={{
                            width: "100%",
                            padding: "6px",
                        ...
                        >
                          {isSelected ? "Currently Loaded" : "Load Sheet"}
                        </button>
                      </div>
                    );
                  })
      ...
      {/* ══════════════════════════════════
          MODAL 3 — History Lookup Dialog
      ══════════════════════════════════ */}
      {showHistoryModal && (
        <Modal title="📋 Past Historical Sheets Directory" onClose={() => setShowHistoryModal(false)}>
                    ...
                    >
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                          <span style={{ fontSize: 14, fontWeight: 800, color: "#1c1917" }}>📅 {sheet.sheetDate}</span>
                          {sheet.isLocked ? (
                            <Badge children="Archived" color="#047857" bg="#d1fae5" />
                          ) : (
                            <Badge children="Draft" color="#b45309" bg="#fef3c7" />
                      ...
                      <button
                        onClick={() => {
                          setSelectedDate(sheet.sheetDate);
