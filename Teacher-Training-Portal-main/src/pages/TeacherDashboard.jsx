...
function OverviewTab({ user, setActiveTab, overviewData = {} }) {
  const profile = overviewData.teacher || user;
  const summary = overviewData.summary || {};
  const monthlyAttendance = overviewData.monthlyAttendance?.length ? overviewData.monthlyAttendance : MOCK_ATTENDANCE_MONTHLY;
  const courses = overviewData.courses?.length ? overviewData.courses : MOCK_COURSES;
  const schedule = overviewData.schedule?.length ? overviewData.schedule : MOCK_SCHEDULE;
  const assignments = overviewData.assignments?.length ? overviewData.assignments : MOCK_ASSIGNMENTS;
  const certificates = overviewData.certificates?.length ? overviewData.certificates : MOCK_CERTIFICATES;

  const attendance = summary.attendance ?? profile.attendance ?? 90;
  ...
  return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      <div style={{ background: "linear-gradient(135deg,#f59e0b,#d97706)", borderRadius: 20, padding: "24px 28px", marginBottom: 24, color: "white", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 900, margin: "0 0 6px", letterSpacing: "-0.3px" }}>Good morning, {profile.name?.split(" ")[0]}! 👋</h1>
          <p style={{ fontSize: 13, margin: 0, opacity: 0.88 }}>{profile.subject} Teacher · {profile.batch || "SpacECE"} · {new Date().toLocaleDateString("en-IN",{weekday:"long",day:"numeric",month:"long"})}</p>
        </div>
      ...
      <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 8, fontSize: 14, fontWeight: 700 }}>
        ...
        <span>Working Center : {profile.workingCenter || "Dhayri, Pune, Maharashtra"}</span>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(160px,1fr))", gap: 16, marginBottom: 24, marginTop: 16 }}>
        <StatCard icon="📚" label="My Classes"     val={summary.myClasses ?? profile.classes ?? 0}   color="#f59e0b" bg="#fef3c7"/>
        <StatCard icon="👥" label="Total Students" val={summary.totalStudents ?? profile.students ?? 0} color="#3b82f6" bg="#dbeafe"/>
        <StatCard icon="📊" label="Attendance"     val={`${attendance}%`}  color={attColor} bg={attendance>=85?"#d1fae5":attendance>=70?"#fef3c7":"#fee2e2"}/>
        <StatCard icon="📝" label="Avg Grade"      val={`${summary.avgGrade ?? 0}%`}               color="#8b5cf6" bg="#ede9fe"/>
        <StatCard icon="🏆" label="Certificates"   val={summary.certificates ?? certificates.length} color="#06b6d4" bg="#cffafe"/>
        <StatCard icon="⏳" label="Pending Tasks"  val={summary.pendingTasks ?? assignments.filter(a=>a.status==="pending"||a.status==="revision").length} color="#ef4444" bg="#fee2e2"/>
      </div>
          ...
          {courses.map((c,i)=>(
            <div key={i} style={{ marginBottom: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 12, fontWeight: 600, color: "#374151" }}>{(c.title || c.name || "Course").split(" ").slice(0,3).join(" ")}...</span>
...
function ProfileTab({ user, onWorkingCenterChange, onUserUpdate }) {
  ...
  const [form, setForm] = useState({
    ...
    workingCenter: user.workingCenter || "",
    subject:       user.subject       || "",
    degree:        user.degree        || "M.Sc. Computer Applications",
    university:    user.university    || "University of Pune",
    netStatus:     user.netStatus     || "UGC NET Qualified",
    netDesc:       user.netDesc       || "Assistant Professor Eligibility",
    expYears:      user.expYears      || "3+ Years Active",
    expBio:        user.expBio        || "Senior Pre-Primary Core Instructor & Curriculum Designer specializing in childhood developmental tracking logic and technology-based pedagogy framework."
  });
        ...
        .then(data => {
          if (data && !data.error) {
            const loaded = {
              ...
              workingCenter: data.workingCenter || "",
              subject:       data.subject       || "",
              degree:        data.degree        || "M.Sc. Computer Applications",
              university:    data.university    || "University of Pune",
              netStatus:     data.netStatus     || "UGC NET Qualified",
              netDesc:       data.netDesc       || "Assistant Professor Eligibility",
              expYears:      data.expYears      || "3+ Years Active",
              expBio:        data.expBio        || "Senior Pre-Primary Core Instructor & Curriculum Designer specializing in childhood developmental tracking logic and technology-based pedagogy framework."
            };
            setForm(loaded);
            setSavedForm(loaded);
          }
  ...
  const handlePasswordChange = (e) => {
    ...
    if (!currentPassword || !newPassword || !confirmNewPassword) {
      setToast({ msg: "Please fill all password fields.", type: "error" });
      return;
    }
    ...
    apiFetch(`${API_BASE}/profile/password`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      ...
      .catch(err => {
        ...
        setToast({ msg: "Error updating password.", type: "error" });
      });
  };
  ...
  return (
    <div style={{ animation: "fadeIn 0.3s ease", maxWidth: 640 }}>
      ...
      {/* Change Password */}
      <form onSubmit={handlePasswordChange} style={{ background: "white", borderRadius: 16, padding: "20px 24px", border: "1px solid #f1f5f9", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
        <h3 style={{ fontSize: 14, fontWeight: 800, color: "#1c1917", margin: "0 0 14px" }}>🔒 Change Password</h3>
        <div style={{ marginBottom: 12 }}>
          <label style={S.label}>Current Password</label>
          <input style={S.input} type="password" placeholder="••••••••" value={passwords.currentPassword} onChange={e => setPasswords({ ...passwords, currentPassword: e.target.value })} />
        </div>
        <div style={{ marginBottom: 12 }}>
          <label style={S.label}>New Password</label>
          <input style={S.input} type="password" placeholder="••••••••" value={passwords.newPassword} onChange={e => setPasswords({ ...passwords, newPassword: e.target.value })} />
        </div>
        <div style={{ marginBottom: 12 }}>
          <label style={S.label}>Confirm New Password</label>
          <input style={S.input} type="password" placeholder="••••••••" value={passwords.confirmNewPassword} onChange={e => setPasswords({ ...passwords, confirmNewPassword: e.target.value })} />
        </div>
        <button type="submit" style={{ ...S.primaryBtn, marginTop: 4 }}>Update Password</button>
      </form>
    </div>
  );
}

function NotificationsTab({ notifications: initialNotifications = [], teacherId }) {
  const [notifications, setNotifications] = useState(initialNotifications.length ? initialNotifications : MOCK_NOTIFICATIONS);
  ...
  const markAll = () => {
    ...
    if (teacherId) {
      fetch(`${API_BASE}/notifications/${teacherId}/mark-all-read`, { method: "PUT" }).catch(() => {});
    }
  };
...
═══════════════════════════════════════════ */
export default function TeacherDashboard({ user: initialUser, onLogout }) {
  ...
  const teacherId = initialUser?._id || initialUser?.id;

  useEffect(() => {
    setUser(initialUser);
    setWorkingCenter(initialUser.workingCenter || "Dhayri, Pune, Maharashtra");
  }, [initialUser]);
  ...
  useEffect(() => {
    ...
    apiFetch(`${API_BASE}/dashboard/overview`)
      .then(res => res.json())
      .then(data => {
        if (!active || data?.error) return;
        setOverviewData(data);
        if (data.teacher) {
          setUser(prev => ({ ...prev, ...data.teacher }));
          setWorkingCenter(data.teacher.workingCenter || "Dhayri, Pune, Maharashtra");
        }
      })
      .catch(() => {});
  ...
  const unreadCount = overviewData.notifications ? overviewData.notifications.filter(n => !n.read).length : MOCK_NOTIFICATIONS.filter(n=>!n.read).length;
  const pendingTaskCount = overviewData.summary?.pendingTasks ?? MOCK_ASSIGNMENTS.filter(a=>a.status==="pending"||a.status==="revision").length;
  ...
  const navItems = [
    ...
    { key: "assignments",   label: "Assignments",         icon: "✏️", badge: pendingTaskCount },
  ...
  const renderContent = () => {
    switch(activeTab) {
      case "overview":      return <OverviewTab user={enrichedUser} setActiveTab={setActiveTab} overviewData={overviewData}/>;
      ...
      case "notifications": return <NotificationsTab notifications={overviewData.notifications || []} teacherId={teacherId}/>;
      case "profile":       return <ProfileTab user={enrichedUser} onWorkingCenterChange={setWorkingCenter} onUserUpdate={setUser}/>;
      default:              return null;
    }
  };
