...
function LoginForm({ onLogin, onGoRegister, onGoForgot }) {
  ...
  const handleLogin = async (e) => {
    ...
    // Handle admin login specially (for demo purposes, in production this should be handled by backend)
    if (email === ADMIN_CREDENTIALS.email && password === ADMIN_CREDENTIALS.password) {
      tokenManager.setTokens(
        "admin_mock_access_token",
        "admin_mock_refresh_token",
        { role: "admin", name: "Admin", email }
      );
      onLogin({ role: "admin", name: "Admin", email });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        setToast({ msg: data.error || "Incorrect credentials. Please try again.", type: "error" });
        return;
      }
      ...
      if (teacher.status === "pending")  { 
        setToast({ msg: "Your account is pending admin approval.", type: "error" }); 
        return; 
      }
      if (teacher.status === "rejected") { 
        setToast({ msg: "Your account has been rejected.", type: "error" }); 
        return; 
      }
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:5001/api/auth/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, email, phone, address, subject, password })
      });
      const data = await response.json();
      if (!response.ok) {
